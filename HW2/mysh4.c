...
#define ANSI_COLOR_CYAN  "\x1b[32m"
...
{
	printf (ANSI_COLOR_PURPLE "%s@" ANSI_COLOR_CYAN "%s%s " ANSI_COLOR_RESET, getlogin(), hostname, PROMPT);
}
...
void exec_pipes(int pipec, int argc, char* argv[pipec][argc]) 
{
	int pindex = 0;
	
	for (pindex; pindex < pipec-1; pindex++)
	{
	...
	if (!fork()) {
		...
		execvp(argv[pindex][0], argv[pindex]);			
...
int main(void)
{
	...
	for(;;)
	{
		...
		if (pid > 0)
		{
			...
			if (condition == 0) break;//exit for args[0] = "exit"
